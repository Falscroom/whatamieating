...
class SeedDatabaseCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            $user1
                ->chooseMeal($meal1, Date::fromDateTime(new DateTimeImmutable()), MealType::BREAKFAST)
                ->addAddition($addition1)
                ->addAddition($addition2)
            ;

            $user1
                ->chooseMeal($meal4, Date::fromDateTime(new DateTimeImmutable()), MealType::LUNCH)
                ->addAddition($addition3)
                ->addAddition($addition4)
            ;

            $user2
                ->chooseMeal($meal2, Date::fromDateTime(new DateTimeImmutable()), MealType::LUNCH)
                ->addAddition($addition2)
                ->addAddition($addition5)
            ;

            $user2
                ->chooseMeal($meal5, Date::fromDateTime(new DateTimeImmutable()), MealType::BREAKFAST)
                ->addAddition($addition1)
                ->addAddition($addition3)
            ;

            $user3
                ->chooseMeal($meal3, Date::fromDateTime(new DateTimeImmutable()), MealType::BREAKFAST)
                ->addAddition($addition4)
                ->addAddition($addition5)
            ;
